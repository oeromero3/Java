...
public class PagoDaoImpl extends AbstractDAO implements PagoDao {
    ...
    public void update(PagoPk pk, Pago dto) throws PagoDaoException {
        ...
        try {
            ...
            stmt.setString(index++, dto.getTipoPago());
            stmt.setString(index++, dto.getNumeroTarjeta());
            stmt.setInt(4, pk.getFacturaIdFactura());
            int rows = stmt.executeUpdate();
            reset(dto);
            long t2 = System.currentTimeMillis();
            System.out.println(rows + " rows affected (" + (t2 - t1) + " ms)");
    ...
    /**
     * Returns the rows from the pago table that matches the specified
     * primary-key value.
     */
     ...
     * 'Factura_ID_Factura = :facturaIdFactura'.
     */
    ...
    /**
     * Returns all rows from the pago table that match the criteria 'Tipo_Pago =
     * :tipoPago'.
     */
    ...
    /**
     * Returns all rows from the pago table that match the specified arbitrary
     * SQL statement
     */
